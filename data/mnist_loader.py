...
class MnistColored(BaseDataLoader):
    def __init__(self, args, list_train_domains, mnist_subset, root, transform=None, data_case='train', match_func=False, download=True):
    ...
    def _get_data(self):
        assert self.args.dataset_name == "colored_mnist"
...
#             self.list_train_domains= ['30', '45']
        ...
        for domain in self.list_train_domains:
                ...
                if domain == "W": # white
                    ...
                    if "R" in domain:
                        while True:
                            ...
                            if alpha > 0.2: break
                        img_colored[0] = mnist_imgs[i].float() * alpha
                    if "G" in domain:
                        while True:
                            alpha = np.random.rand()
                            if alpha > 0.2: break
                        img_colored[1] = mnist_imgs[i].float() * alpha 
                    if "B" in domain:
                        while True:
                            alpha = np.random.rand()
                            if alpha > 0.2: break
                        img_colored[2] = mnist_imgs[i].float() * alpha
            ...
            print('Source Domain ', domain)
            training_list_img.append(mnist_img_colored)
            training_list_labels.append(mnist_labels)
            training_list_idx.append(mnist_idx)
            training_list_size.append(mnist_img_colored.shape[0])
