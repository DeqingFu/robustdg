...
class MnistColoredAugEval(BaseDataLoader):
    def __init__(self, args, list_train_domains, mnist_subset, root, transform=None, data_case='train', match_func=False, download=True):
    ...
    def _get_data(self):
        ...
        # train: domain: ["R", "G", "B", "RG", "RB", "GB", "RGB"]
        # test: domain: ["W"]
        image_counter=0
        for domain in self.list_train_domains:
            # Run transforms
            mnist_img_colored= torch.zeros((mnist_size, self.args.img_c, self.args.img_w, self.args.img_h))
            mnist_img_colored_org= torch.zeros((mnist_size, self.args.img_c, self.args.img_w, self.args.img_h))
            mnist_idx=[]
            ...
            for i in range(len(mnist_imgs)):
                ...
                mnist_img_colored_org[i] = to_tensor(img_colored)

                mnist_idx.append( image_counter )
                image_counter+= 1                
            
            print('Source Domain ', domain)
            training_list_img['aug'].append(mnist_img_colored)            
            training_list_img['org'].append(mnist_img_colored_org)      
            ...
            training_list_size['aug']+= mnist_img_colored.shape[0]
            training_list_size['org']+= mnist_img_colored.shape[0]    
